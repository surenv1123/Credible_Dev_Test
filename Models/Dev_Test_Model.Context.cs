//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Credible_Dev_Test.Models
{
    using System;
    using System.Data.Entity;
    using System.Data.Entity.Infrastructure;
    
    public partial class dev_testEntities : DbContext
    {
        public dev_testEntities()
            : base("name=dev_testEntities")
        {
        }
    
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            throw new UnintentionalCodeFirstException();
        }
    
        public virtual DbSet<u_course_portal> u_course_portal { get; set; }
        public virtual DbSet<u_portal> u_portal { get; set; }
        public virtual DbSet<u_registration> u_registration { get; set; }
        public virtual DbSet<u_user> u_user { get; set; }

        public virtual DbSet<RegisteredUserInfo> RegisteredUserInfo { get; set; }
    }
}
